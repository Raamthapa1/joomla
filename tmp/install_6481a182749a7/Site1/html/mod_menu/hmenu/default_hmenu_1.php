    ...
    function buildMenu1($list, $default_id, $active_id, $path, $options)
	{
        ...
        <ul class="<?php echo $options['menu_class']; ?>">
            <?php foreach ($list as $i => &$item) {
                ...
                if ($item->level > 2 && $item->isMegaMenu) {
                ...
                if ($item->deeper) {
                    if ($item->level == 1 && $item->isMegaMenu) {
                        echo '<!--start_megapopup_' . $popupKey .'-->';
                    } else {
                        if ($item->isMegaMenu && $item->level == 2) {
                    ...
                    if ($item->isMegaMenu && $item->level == 2) {
                        ...
                        $options['submenu_class'] = $isMegaMenu && !$item->isMegaMenu
                ...
                elseif ($item->shallower) {
                    ...
                    $closeDiff = '';
                    if ($item->isMegaMenu && $item->needMegaMenuDiff) {
...
$menu_html = buildMenu1($mainList, $default_id, $active_id, $path, array(
		...
		'submenu_class' => 'u-h-spacing-20 u-nav u-unstyled u-v-spacing-10 u-block-aaad-19',
...
$resp_menu = buildMenu1($list, $default_id, $active_id, $path, array(
        ...
        'link_style' => '',
		'submenu_class' => 'u-h-spacing-20 u-nav u-unstyled u-v-spacing-10 u-block-aaad-17',
		'submenu_item_class' => 'u-nav-item',
		'submenu_link_class' => 'u-button-style u-nav-link u-white',
        'submenu_link_style' => ''
	)
